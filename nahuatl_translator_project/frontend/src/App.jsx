...
function Header(){
  return (
    <div className="card">
      <h1 style={{margin:'0 0 6px 0'}}>Nahuatl Translator & Manuscript Transcriber</h1>
      <div className="status">
        Frontend: GitHub Pages • Backend API: <code>{API_BASE}</code>
      </div>
      <p style={{margin:'10px 0 0 0',opacity:.85}}>
        Local-first can be added later, but this build is optimized for <b>OpenAI translation + vision transcription</b>,
        with <b>translation variants</b> for researchers.
      </p>
    </div>
  )
}
...
function TranslateTab(){
  ...
  async function run(){
    ...
    setOut(null)
    try{
      const res = await postJSON('/api/translate', {text,src,tgt,variety,variants:Number(variants),temperature:Number(temperature)})
      setOut(res)
    }catch(e){
      setErr(e.message)
    }finally{
    ...
    <div className="card">
      ...
      <div className="row">
        <div>
          ...
          <input type="number" min="1" max="5" value={variants} onChange={e=>setVariants(e.target.value)} />
        ...
        <div style={{display:'flex',alignItems:'end',gap:10}}>
          ...
          <button className="secondary" onClick={()=>{setText('');setOut(null);setErr('')}}>
            Clear
          </button>
        </div>
      </div>
...
function TranscribeTab(){
  ...
  async function run(){
    ...
    try{
      ...
      const res = await postForm('/api/transcribe', form)
      setOut(res)
    }catch(e){
      setErr(e.message)
    }finally{
  ...
  return (
    <div className="card">
      ...
      <textarea value={alphabetHint} onChange={e=>setAlphabetHint(e.target.value)} placeholder="Paste Translating Mesoamerica alphabet rules here (optional)…" />

      <div className="row">
        <div>
          <label>Handwriting profile</label>
          <select value={profile ? 'yes' : 'no'} onChange={e=>setProfile(e.target.value==='yes')}>
...
function ExtractTab(){
  ...
  async function run(){
    setErr(''); setOut(null); setLoading(true)
    try{
      const res = await postJSON('/api/extract', {text, instruction, schema_hint: schema})
      setOut(res)
    }catch(e){
      setErr(e.message)
    }finally{
    ...
    <div className="card">
      ...
      <div className="row">
        ...
        <div>
          ...
          <input value={schema} onChange={e=>setSchema(e.target.value)} placeholder='e.g., {"people":[],"dates":[]}' />
        </div>
      </div>
      <label>Text</label>
      <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Paste transcription or document text…" />

      <div style={{marginTop:10,display:'flex',gap:10}}>
        <button onClick={run} disabled={loading || !text.trim()}>{loading ? 'Extracting…' : 'Extract'}</button>
        <button className="secondary" onClick={()=>{setText('');setOut(null);setErr('')}}>Clear</button>
      </div>
  ...
  return (
    <div className="container">
      ...
      <div className="card">
        ...
        <ul>
          <li><b>LLM vs Transkribus:</b> measure CER/WER on a gold set; segment by scan quality, scribal hand, and language mixture.</li>
          <li><b>Translating Mesoamerica alphabet:</b> paste orthography rules above to constrain transcription behavior; later we can add automated validation.</li>
          <li><b>Scribes:</b> handwriting profile output can be clustered across pages as a starting point for scholar review.</li>
        </ul>
      </div>
    </div>
  )
